...
SIMULATION <- function(N = 100, MaxYear = 10, h2 = matrix(c(0.5,0.0, 0, .3), nrow = 2, ncol = 2, byrow = T), mu = c(10, 0.5), P = matrix(c(20, 0, 0, 0.1), byrow = T, nrow = 2, ncol = 2), Ag = 5, Bg = -0.5)
{
  ...
  #set up array
  Individuals <- array(0,dim = c(N,13,MaxYear)) #All individuals saved in matrix
  colnames(Individuals) <- c("ID", "BirthPhen", "GrowPhen", "BirthGen", "GrowGen", "BirthEnv", "GrowEnv", "Sex", "Fitness", "Mutation", "Growth", "Alive", "Age")
  ...
  # Phenotype values
  # Phenotypic values (mean(average of parents) + genetic intercept + environmental intercept + genetic slope + environmental slope)*year
  ...
  lambda <- P*N #mean number of mutations in population
  N.mutations    <- rpois(1,lambda)
  rows <- sample(1:N, N.mutations)
  sapply(rows, MUTATION)   # Apply function MUTATION to generate mutant loci
  ...
  for (Iyear in 2:MaxYear)          # Iterate over years
  {
    ...
    # all live 
      ...
      Individuals[,"Phenotype",Iyear] <- Individuals[,"GrowPhen",Iyear] <- (Individuals[,"BirthPhen",Iyear] + Individuals[,"BirthGen",Iyear] + Individuals[,"GrowGen",Iyear] + Individuals[,"BirthEnv",Iyear] + Individuals[,"GrowEnv",Iyear])*Individuals[,"Age",Iyear]
      ...
      Iyear_moms = Individuals[which(Individuals[,"Sex",Iyear-1] == TRUE & Individuals[,"Alive",Iyear-1] == TRUE),"GrowGen",Iyear-1] + Individuals[which(Individuals[,"Sex",Iyear-1] == TRUE & Individuals[,"Alive",Iyear-1] == TRUE),"Mutation",Iyear-1]
      Iyear_dads = Individuals[which(Individuals[,"Sex",Iyear-1] == FALSE & Individuals[,"Alive",Iyear-1] == TRUE),"GrowGen",Iyear-1] + Individuals[which(Individuals[,"Sex",Iyear-1] == FALSE & Individuals[,"Alive",Iyear-1] == TRUE),"Mutation",Iyear-1]
      ...
      # Mutation values
      # Number of mutations using a Poisson distribution
      lambda         <- 2*P*(N-num_alive) #mean number of mutations in population
      N.mutations    <- rpois(1,lambda)
      rows <- sample(N, N.mutations)
      sapply(rows, MUTATION, Iyear = Iyear) # Apply function MUTATION to generate mutant loci
      
      # Sex of Offspring
      Individuals[(num_alive+1):N,"Sex",Iyear] <- sample(c(0,1), N-num_alive, replace = T) #set all sexes
      
      # Environmental values
      Individuals[,"BirthEnv",Iyear] 	  	<- Individuals[,"BirthGen",Iyear] + grfx(N, E, warn = F)   # Environmental values
      Individuals[,"GrowEnv",Iyear] 	  	<- Individuals[,"GrowGen",Iyear] + grfx(N, E, warn = F)   # Environmental values 
